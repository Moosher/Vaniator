<?php
	...
	while(!feof($cnpj_list)){
		...
		if($line_read{1} == 1){		
			...
			$MDBcollection = $cnpjSC->cnpjsc;
			$validacao = $MDBcollection->findOne(['cnpj' => $line_validation]);
	...
	function connect($proxyC){
		...
		global $i, $line_read, $proxy, $cnpjSC;
		...
		$documento = json_decode($retorno_json);		

		if($documento != null){

			$cnpjSC->cnpjsc->insertOne($documento);
